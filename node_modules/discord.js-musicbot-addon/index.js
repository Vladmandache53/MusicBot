...
try {
    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          help: (options && options.play && options.play.help) || "pune-ma sa pun muzica pe voce .",
        ...
        this.help = {
          ...
          help: (options && options.help && options.help.help) || "Help pentru comenzi.",
        ...
        this.pause = {
          ...
          help: (options && options.pause && options.pause.help) || "da pauza la muzica.",
        ...
        this.resume = {
          ...
          help: (options && options.resume && options.resume.help) || "Resumeaza o musica oprita.",
        ...
        this.leave = {
          ...
          help: (options && options.leave && options.leave.help) || "Iese din voice.",
        ...
        this.queue = {
          ...
          help: (options && options.queue && options.queue.help) || "Uitete la queue-ul la server.",
        ...
        this.np = {
          ...
          help: (options && options.np && options.np.help) || "arata ce se canta acum.",
        ...
        this.loop = {
          ...
          help: (options && options.loop && options.loop.help) || "seteaza loop pentru queue-ul acesta.",
        ...
        this.search = {
          ...
          help: (options && options.search && options.search.help) || "Cauta pana la  10 video-uri pe htpss://Youtube.com.",
        ...
        this.clearqueue = {
          ...
          help: (options && options.clear && options.clear.help) || "Sterge musica din queue-ul respectiv.",
        ...
        this.volume = {
          ...
          help: (options && options.volume && options.volume.help) || "Steaza volum-ul pe bot.",
        ...
        this.remove = {
          ...
          help: (options && options.remove && options.remove.help) || "scoate o melodie din queue-ul respectiv.",
        ...
        this.skip = {
          ...
          help: (options && options.skip && options.skip.help) || "trece peste melodia curenta",
        ...
        this.deleteQueue = {
            ...
            help: (options && options.deleteQueue && options.deleteQueue.help) || "Ștergeți și faceți din nou o coadă continuă",
      ...
      checkVoice(mem, bot) {
        return new Promise((resolve, reject) => {
          ...
          if (!mem.voiceChannel) reject("Nu esti intr-un canal!");
          if (bot.voiceChannel) {
            if (bot.voiceChannel.id == mem.voiceChannel.id) resolve(mem.voiceChannel)
            else reject("Esti intr-un canal diferit de almeu!")
          } else {
      ...
      async updatePositions(obj, server) {
        return new Promise((resolve, reject) => {
          ...
          if (obj.working == true) reject("Server-ul deja are o lista activ!");
          if (server != "000000") {
            obj.working = true;
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un canal de voice.`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'Nu ai zis nici un video!'));
      let q = musicbot.getQueue(msg.guild.id);

      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Trebuie sa fi in acelasi canal cu mine.`));
      if (q.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', 'queue maxim atins!'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        ...
        ytpl(playid, {limit: musicbot.maxQueueSize}, function(err, playlist) {
          if(err) return msg.channel.send(musicbot.note('fail', `Ceva a mers gresit cand luam playlist-ul!`));
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `Nu pot lua nici un video din acel playlist.`));
          if (playlist.total_items >= musicbot.maxQueueSize && musicbot.maxQueueSize != 0) return msg.channel.send(musicbot.note('fail', `Prea multe video-uri. Maxim-ul de ` + musicbot.maxQueueSize + ` este permis.`));
          var index = 0;
          var ran = 0;
          ...
          playlist.items.forEach(async (video) => {
            ...
            if (ran >= playlist.items.length) {
              ...
              if (index == 0) msg.channel.send(musicbot.note('fail', `Nu am putut sa iau nici un video din acel playlist`))
        ...
        new Promise(async (resolve, reject) => {
          let result = await musicbot.searcher.search(searchstring, { type: 'video' }).catch((err) => {
            ...
            if (errorMsg.includes('\"reason\": \"dailyLimitExceeded\",')) {
              errorMsg = errorMsg.slice(errorMsg.indexOf('limita zilnica depasita!'));
              errorMsg = errorMsg.slice(0, errorMsg.indexOf('\",'));
              if (!ignore) msg.channel.send(musicbot.note("fail", "**imposibil de completat playlist-ul:**\n" + errorMsg));
              return;
            } else if (errorMsg.includes('\"reason\": \"quotaExceeded\",')) {
              if (!ignore) msg.channel.send(musicbot.note("fail", "Imposibil de completat redarea! Cora API-ului Google a fost depășită!"));
              return;
            } else {
              if (!ignore) msg.channel.send(musicbot.note("fail", "A apărut o eroare necunoscută! Redarea nu a putut fi finalizată, verificați jurnalele pentru mai multe detalii."));
          ...
          if (!res) return msg.channel.send(musicbot.note("fail", "Ceva n-a mers bine. Încearcă din nou!"));
          ...
          if (!ignore) {
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              ...
              try {
                embed.setAuthor('Se adauga in lista!', client.user.avatarURL);
                ...
                if (musicbot.requesterName && resMem) embed.setFooter(`ceruta de ${client.users.get(res.requester).username}`, res.requesterAvatarURL);
                if (musicbot.requesterName && !resMem) embed.setFooter(`ceruta de \`UnknownUser (ID: ${res.requester})\``, res.requesterAvatarURL);
                msg.channel.send({
                  embed
                });
              } catch (e) {
              ...
              try {
                ...
                .replace(/~/g, '\\~')
                .replace(/`/g, '\\`');
                msg.channel.send(`Se canta acum: **${songTitle}**\nSolicitat de: ${client.users.get(res.requester).username}\Pus în coadă: ${res.queuedOn}`)
              } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          var cmdmsg = `= Music Commands =\nUtilizeaza ${prefix}${musicbot.help.name} [command] pentru ajutor la o commanda. Oricine are un rol numit \`${musicbot.djRole}\` poate folosi orice comanda.\n`;
          let index = 0;
          let max = musicbot.commandsArray.length;
          for (var i = 0; i < musicbot.commandsArray.length; i++) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          command = musicbot.commands.get(command) || musicbot.aliases.get(command);
          if (command.exclude) return msg.channel.send(musicbot.note('fail', `${suffix} nu este o comanda valida!`));
          var cmdhelp = `= ${command.name} =\n`;
          cmdhelp = cmdhelp + `\n${command.help}`;
          if (command.usage !== null) cmdhelp = cmdhelp + `\nFolosire: ${command.usage.replace(/{{prefix}})/g, prefix)}`;
          if (command.alt.length > 0) cmdhelp = cmdhelp + `\n
Pseudonime: ${command.alt.join(", ")}`;
          msg.channel.send(cmdhelp, {
            code: 'asciidoc'
          });
        };
      } else {
        msg.channel.send(musicbot.note('fail', `${suffix} nu este o comanda valida!`));
      };
    };

    musicbot.skipFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un canal de voice.`));
    ...
    musicbot.pauseFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un voice.`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No music being played.'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (dispatcher.paused) return msg.channel.send(musicbot.note(`fail`, `Muzica este deja oprita!`))
    ...
    musicbot.resumeFunction = (msg, suffix, args) => {
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher.paused) return msg.channel.send(musicbot.note('fail', `Muzica deja canta.`))
    ...
    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        ...
        if (chance <= 10) return msg.channel.send(musicbot.note('fail', `Nu te pot lasa sa faci asta, ${msg.author.username}.`))
        else return msg.channel.send(musicbot.note('fail', 'Scuze, tu nu ai acces la acel lucru.'));
      }
    }

    musicbot.npFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Nici o musica nu e pusa! dece nu pui tu cu `@play [nume melodie]`?.'));
      const queue = musicbot.getQueue(msg.guild.id, true);
      const dispatcher = voiceConnection.player.dispatcher;

      if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', 'Lista e goala, Dece nu adaugi ceva?'));
      ...
      if (msg.channel.permissionsFor(msg.guild.me)
        ...
        try {
          var songTitle = queue.last.title.replace(/\\/g, '\\\\')
            ...
            .replace(/~/g, '\\~')
            .replace(/`/g, '\\`');
          msg.channel.send(`Se canta: **${songTitle}**\nData de: ${client.users.get(queue.last.requester).username}\nPusa pe lista ca numarul: ${queue.last.queuedOn}`)
        } catch (e) {
          console.error(`[${msg.guild.name}] [npCmd] ` + e.stack);
        };
      }
    ...
    musicbot.deleteQueueFunction = async (msg, suffix, args) => {
      if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", "Doar un admin poate face asta."));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      musicbot.emptyQueue(msg.guild.id).then(() => {
        ...
        return msg.channel.send(musicbot.note("note", "Lista acum este goala."))
      }).catch(async (res) => {
        ...
        msg.channel.send(musicbot.note("note", "Lista trebuie sa fie goala."))
      })
    }
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      ...
      if (suffix) {
        ...
        if (!video) return msg.channel.send(musicbot.note("fail", "Nu am gasit acel video."));
        const embed = new Discord.RichEmbed()
            ...
            .then(searchResult => {
              ...
              const startTheFun = async (videos, max) => {
                if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                  ...
                  embed.setFooter(`Cautat de: ${msg.author.username}`, msg.author.displayAvatarURL);
                  msg.channel.send({
                    embed
                  })
                  .then(firstMsg => {
    ...
    musicbot.volumeFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un canal.`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Nici o musica nu e pusa pe linie! dece nu pui tu cu `@play [video]` .'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Trebuie sa fii in acelasi canal cu mine.`));
      if (!musicbot.canAdjust(msg.member, musicbot.queues.get(msg.guild.id))) return msg.channel.send(musicbot.note('fail', `Doar Admini si Dj poate sa foloseasca aceasta cmd.`));
      const dispatcher = voiceConnection.player.dispatcher;

      if (!suffix || isNaN(suffix)) return msg.channel.send(musicbot.note('fail', 'Nici un volum nu a fost zis.'));
      suffix = parseInt(suffix);
      if (suffix > 200 || suffix <= 0) return msg.channel.send(musicbot.note('fail', 'Volum prea mare sau prea mic,alege intre 1 si 200'));
    ...
    musicbot.removeFunction = (msg, suffix, args) => {
      ...
      if (test) {
        ...
        msg.channel.send(musicbot.note("fail", "Nu am putut gasi acel video. ceva a mers gresit."));
      }
    };

    musicbot.loopFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un canal.`));
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', `Nici o lista nu a fost gasit pe server!`));
      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Trebuie sa fii in acelasi canal cu mine.`));
      if (musicbot.queues.get(msg.guild.id).loop == "none" || musicbot.queues.get(msg.guild.id).loop == null) {
        ...
        msg.channel.send(musicbot.note('note', 'Repetarea a fost pornita [Mod: Single]! :repeat_one:'));
      } else if (musicbot.queues.get(msg.guild.id).loop == "song") {
        musicbot.queues.get(msg.guild.id).loop = "queue";
        msg.channel.send(musicbot.note('note', 'Repetarea a fost pornita [Mod: Single]! :repeat:'));
      } else if (musicbot.queues.get(msg.guild.id).loop == "queue") {
        musicbot.queues.get(msg.guild.id).loop = "none";
        msg.channel.send(musicbot.note('note', 'Repetarea a fost oprita!, acum se contiuna fara looping! :arrow_forward:'));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        const dispatcher = voiceConnection.player.dispatcher;
    ...
    musicbot.shuffleFunction = (msg, suffix, args) => {
      let q = musicbot.getQueue(msg.guild.id);
      if (q.working == true) return msg.channel.send(musicbot.note('fail', `Acest server are deja o lista care este pusa in functiune!`));
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nu esti intr-un canal de voce.`));
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', `Nici o lista pentru acest sv nu a fost gasita!!`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Trebuie sa fii in acelasi canal ca mine!.`));
      ...
      if (musicbot.queues.get(msg.guild.id).loop == "song") return msg.channel.send(musicbot.note("fail", `Nu se poate da ``shuffle`` cand ``Repetarea`` este pe ``Mod[SINGLE]``.`));
      const dispatcher = voiceConnection.player.dispatcher;
      q.oldSongs = q.songs;
      ...
      musicbot.updatePositions(q, msg.guild.id).then((res) => {
        ...
        message.channel.send(musicbot.note("fail", "Ceva a mers gresit cand am incercat sa dau shuffle la lista!"))
      ...
      new Promise((resolve, reject) => {
          ...
          if (voiceConnection === null) {
            if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
              ...
              msg.channel.send(musicbot.note('fail', 'Nu am permisie de a intra pe acel canal :cry:'))
              reject();
            } else {
              musicbot.emptyQueue(msg.guild.id).then(() => {
                reject();
          ...
          if (!video) {
            ...
            if (!video) {
              msg.channel.send(musicbot.note('note', 'Lista s-a terminat!'));
          ...
          try {
            ...
            connection.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Ceva a mers gresit cu conexiunea la servere cu ``Discord``. Se ia lista...`));
              musicbot.executeQueue(msg, queue);
            });

            dispatcher.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Ceva a mers gresit cand cantam pentru voi. Se ia lista...`));
              musicbot.executeQueue(msg, queue);
            });
